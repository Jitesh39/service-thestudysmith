...
export default function Navbar() {
    ...
    // Body Lock & Global state for mobile menu
    useEffect(() => {
        if (isMobileMenuOpen) {
    ...
    useEffect(() => {
        ...
        const broadcastId = userRole === "Team_Member" ? "all_team" : "all_client";
        const q = query(
            collection(db, "notifications"),
            where("receiverId", "in", [user.uid, broadcastId]),
            where("isRead", "==", false)
        );
    ...
    return (
        <>
            ...
            {isMobileMenuOpen && (
                <div
                    className="fixed inset-0 z-40 bg-white/10 backdrop-blur-md animate-in fade-in duration-300 md:hidden"
            ...
            >
                <div className="container flex items-center justify-between h-14 md:h-16 relative z-50">
                    ...
                    <div className="flex items-center gap-1.5 md:gap-4">
                        ...
                        {user && (
                            <button
                                onClick={() => router.push(`${getDashboardHref()}?section=notifications`)}
                                className={cn(
                                    "relative p-2 transition-all hover:scale-110",
                                    isDarkSection ? "text-white hover:text-slate-200" : "text-slate-600 hover:text-primary"
                                )}
                ...
                {/* Mobile Menu */}
                {isMobileMenuOpen && (
                    <div className="md:hidden absolute top-full left-0 right-0 mx-2 mt-2 p-4 bg-white/100 border border-slate-100 rounded-2xl shadow-xl flex flex-col gap-1 animate-in slide-in-from-top-2 fade-in duration-200 z-[60]">
                        {navLinks.map((link) => (
                            <Link
                                key={link.name}
                                href={link.href}
